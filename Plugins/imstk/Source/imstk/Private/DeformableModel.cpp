...
void UDeformableModel::InitializeComponent()
{
	...
	// TODO: add error catching for not having meshes attached
	...
	else if (UStaticMeshComponent* StaticMesh = (UStaticMeshComponent*)Owner->GetComponentByClass(UStaticMeshComponent::StaticClass()))
	{
		...
		ConvertStaticToProceduralMesh(StaticMesh);
...
{
	std::shared_ptr<imstk::SurfaceMesh> MeshGeom = std::dynamic_pointer_cast<imstk::SurfaceMesh>(PbdObject->getVisualGeometry());

	// Update the procedural mesh to positions from imstk
	// Currently only supports vertex positions
	if (MeshComp) {
		TArray<FVector2D> UV0;
		TArray<FLinearColor> VertColors;
		TArray<FProcMeshTangent> Tangents;

		//imstk::VecDataArray<double, 3>& ImstkNorms = *MeshGeom->getVertexNormals().get();
...
void UDeformableModel::Init()
{
	...
	Super::IsInitalized = true;

}

void UDeformableModel::ConvertStaticToProceduralMesh(UStaticMeshComponent* StaticMesh)
{
	StaticMesh->GetStaticMesh()->bAllowCPUAccess = true;
	UKismetProceduralMeshLibrary::CopyProceduralMeshFromStaticMeshComponent(StaticMesh, 0, MeshComp, false);
}

void UDeformableModel::ProcessBoundaryConditions()
{
	if (BoundaryConditionActor == nullptr)
		return;
	// Get the mesh from the assigned variable
	UStaticMesh* BoundaryMesh = ((UStaticMeshComponent*)BoundaryConditionActor->GetComponentByClass(UStaticMeshComponent::StaticClass()))->GetStaticMesh();

	FixedNodes.Empty();
	// TODO: this could be made into a util function
	...
	BoundaryMeshGeom->scale(UMathUtil::ToImstkVec3(BoundaryConditionActor->GetActorScale()), imstk::Geometry::TransformType::ApplyToData);
	BoundaryMeshGeom->rotate(UMathUtil::ToImstkQuat(BoundaryConditionActor->GetActorRotation().Quaternion()), imstk::Geometry::TransformType::ApplyToData);
	BoundaryMeshGeom->updatePostTransformData();
	BoundaryMeshGeom->translate(UMathUtil::ToImstkVec3(BoundaryConditionActor->GetActorLocation()), imstk::Geometry::TransformType::ApplyToData);
	...
	BoundaryConditionActor->Destroy();
}
